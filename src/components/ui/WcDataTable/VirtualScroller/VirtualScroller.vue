<template>
  <template v-if="!disabled">
    ...
    >
      ...
      />
      <div
        v-if="!loaderDisabled && showLoader && d_loading"
        :class="loaderClass"
      >
          ...
          <template v-for="(_, index) of loaderArr" :key="index">
            <slot
              ...
              :options="
                getLoaderOptions(
                  index,
                  isBoth() && { numCols: d_numItemsInViewport.cols },
                )
              "
            />
          </template>
  ...
  export default {
          ...
          if (this.isBoth()) {
            return items
              ...
              .map((item) =>
                this.columns
                  ? item
                  : item.slice(this.first.cols, this.last.cols),
              );
          ...
          if (both || horizontal) {
            return this.d_loading && this.loaderDisabled
              ? both
                ...
                : this.loaderArr
              : this.columns.slice(
                  both ? this.first.cols : this.first,
                  both ? this.last.cols : this.last,
                );
          }
        }
    ...
    methods: {
      ...
      scrollToIndex(index, behavior = 'auto') {
        ...
        if (both) {
          ...
          if (newFirst.rows !== first.rows || newFirst.cols !== first.cols) {
            scrollTo(
              calculateCoord(newFirst.cols, itemSize[1]),
              calculateCoord(newFirst.rows, itemSize[0]),
            );
          }
        } else {
          ...
          if (newFirst !== first) {
            horizontal
              ? scrollTo(calculateCoord(newFirst, itemSize), 0)
              : scrollTo(0, calculateCoord(newFirst, itemSize));
          }
        }
      },
          ...
          if (isToStart) {
            if (both) {
              if (viewport.first.rows - first.rows > index[0]) {
                scrollTo(
                  viewport.first.cols * this.itemSize[1],
                  (viewport.first.rows - 1) * this.itemSize[0],
                );
              } else if (viewport.first.cols - first.cols > index[1]) {
                scrollTo(
                  (viewport.first.cols - 1) * this.itemSize[1],
                  viewport.first.rows * this.itemSize[0],
                );
              }
            } else {
      ...
      calculateNumItems() {
        ...
        const contentWidth = this.element
          ? this.element.offsetWidth - contentPos.left
          : 0;
        const contentHeight = this.element
          ? this.element.offsetHeight - contentPos.top
          : 0;
      ...
      getLast(last = 0, isCols) {
        if (this.items) {
          return Math.min(
            isCols ? (this.columns || this.items[0]).length : this.items.length,
            last,
          );
        }
      ...
      setSize() {
        if (this.element) {
          ...
          const width =
            this.scrollWidth ||
            `${this.element.offsetWidth || parentElement.offsetWidth}px`;
          const height =
            this.scrollHeight ||
            `${this.element.offsetHeight || parentElement.offsetHeight}px`;
